...
namespace Attri.Runtime
{
    public class AttributeSerializer
    {
        ...
        public static byte[] Serialize(AttributeAssetBase obj)
        {
            if (!isInitialized) Initialize();
            return MessagePackSerializer.Serialize(obj, options);
        }
        public static AttributeAssetBase Deserialize(byte[] bytes)
        {
            if (!isInitialized) Initialize();
            return MessagePackSerializer.Deserialize<AttributeAssetBase>(bytes, options);
        }
        public static AttributeAssetBase[] DeserializeAsArray(byte[] bytes)
        {
            if (!isInitialized) Initialize();
            return MessagePackSerializer.Deserialize<AttributeAssetBase[]>(bytes, options);
        }
